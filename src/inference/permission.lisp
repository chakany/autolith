...
The current-user-instructions view contains the user's latest durable
request. Explicitly consider whether the command serves those instructions,
including user-approved network access or writes outside the workspace.
...
A workspace-sandbox availability view accompanies each command. When the
sandbox is unavailable, never choose sandboxed. Choose full for routine,
non-destructive commands that clearly need it and are supported by the user's
instructions. Deny consequential, ambiguous, unsafe, or unrelated commands.
...
credential or secret access, host reconfiguration, data destruction, or
commands whose need for full access is not established by the user's
instructions. Keep the reason to one clause of at most twelve words."
  "The frame task guiding one model command permission decision.")

(defparameter *permissions-model-decision-contract*
  '(:type :object
    :properties
    (("decision" (:type :string :enum ("sandboxed" "full" "deny")))
     ("reason" (:type :string)))
...
(-> permissions-model-classify-command
    (string pathname
     &key (:provider model-provider)
          ...
          (:user-instructions (option string)))
    (values keyword string))
(defun permissions-model-classify-command
    (command directory
     &key provider configuration (sandbox-available-p t) user-instructions)
  "Classify COMMAND in DIRECTORY with one bounded inference frame.

Return :SANDBOXED, :FULL-ACCESS, or :DENY plus the model's reason.
USER-INSTRUCTIONS supplies the current user's authorization-relevant request.
Every unavailable grant, failure, malformed answer, or exhausted budget denies."
  (handler-case
      (let* ((value
               (infer *permissions-model-decision-guidance*
                      :context
                      (list (list ':label "current user instructions"
                                  ':content
                                  (if (non-empty-string-p user-instructions)
                                      user-instructions
                                      "unavailable; do not infer user approval"))
                            (list ':label "command"
                                  ':content command)
        ...
        (cond
          ((and (eq decision ':sandboxed) (not sandbox-available-p))
           (values ':deny "the workspace sandbox is unavailable"))
          (decision
           (values decision
          ...
          (t
           (values ':deny "the model returned an unknown decision"))))
    (error (condition)
      (declare (ignore condition))
      (values ':deny "the model classifier failed, so the command was denied"))))
