...
Authenticate ChatGPT with the browser device flow. Bare =--auth= instead uses the
provider selected by your configuration.

#+BEGIN_SRC sh
autolith --auth chatgpt
#+END_SRC

API-key providers name the provider explicitly. Autolith shows a clearly labeled
hidden field: paste the key, press Enter, and expect no characters to appear.
...
** Lisp-first control
Prose, callable commands, tools, and explicit Common Lisp share the REPL.
...
Steering messages and follow-ups survive active-image crashes in a conversation-scoped vault.
The vault is (intentionally) not resubmitted automatically, since you may want to diagnose
the crash before you resume your previous work.
