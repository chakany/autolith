  ...
  (let* ((registry (make-default-tool-registry))
    ...
    (unwind-protect
         (let* ((conversation (conversation-create configuration
           ...
           (test-assert (= (length (tool-registry-tools registry)) 57)
                        "the default registry exposes the complete initial tool set")
           (test-assert (= (length schemas) 10)
                        "the provider schemas contain ten namespaces")
           ...
           (test-assert (string= (json-get (aref schemas 4) "name") "papercut")
                        "model-visible papercuts have a dedicated namespace")
           (test-assert (= (length (json-get (aref schemas 4) "tools")) 1)
                        "the papercut namespace exposes one reporting operation")
           (test-assert (string= (json-get (aref schemas 5) "name") "agenda")
           ...
           (test-assert (string= (json-get (aref schemas 6) "name") "plan")
                        "session plans have a dedicated namespace")
           (test-assert (= (length (json-get (aref schemas 6) "tools")) 3)
                        "the plan namespace exposes three operations")
           (test-assert (string= (json-get (aref schemas 7) "name") "lisp")
                        "the named Lisp namespace follows the workspace tools")
           (test-assert (= (length (json-get (aref schemas 7) "tools")) 18)
           ...
           (test-assert (string= (json-get (aref schemas 8) "name") "self")
                        "the active-image namespace follows Lisp workers")
           (test-assert (= (length (json-get (aref schemas 8) "tools")) 14)
           ...
           (test-assert (string= (json-get (aref schemas 9) "name") "skill")
           ...
           (let* ((immutable-registry
                  ...
                  (self-schema (aref immutable-schemas 8)))
             (test-assert (= (length (tool-registry-tools immutable-registry)) 48)
